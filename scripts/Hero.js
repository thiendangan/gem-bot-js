...
class SEA_SPIRIT_SKILL extends BaseSkill {
  ...
  getTarget(posibleGemSwaps, posibleSkillCasts, state) {
    const heroAlive = state.getCurrentPlayer().getHerosAlive();
    let enemyPlayer = state.getCurrentEnemyPlayer();
    // let enemyPlayerMaxHp = enemyPlayer.getCurrentMaxHp();
    // let enemyMaxAttack = enemyPlayer.getCurrentMaxAttack() + 2;// cho con cho
    let {hp: enemyPlayerMaxHp, attack: enemyMaxAttack} = enemyPlayer.getCurrentMax();
    enemyMaxAttack += 2;
    // let dontCast = false;
    let dontCast = true;
    ...
    if (temHero && temHero.attack < enemyPlayerMaxHp) {
      // neu hero attack > max enemy ==> k buff
      hero = temHero
    }
    // neu hero attack > max enemy ==> k buff
    if (hero.attack >= enemyPlayerMaxHp) {
      const otherHero = heroAlive.find(h => h != hero && h != temHero);
      if (otherHero) {
        hero = otherHero;
      } else {
          dontCast = true;
          return new SkillTarget(null, dontCast);
      }
    }
    ...
    // if (enemyHero && enemyHero.hasSkill()) {// truong hop xau con seasprit// todo
    //   enemySpritAttack = this.hero.attack + state.grid.getNumberOfGemByType(GemType.RED) + 3;
    // }
    // todo thunder

    // console.error(this.hero.hp, {enemySpritAttack, enemyMaxAttack})
    if (this.hero.hp <= Math.max(enemySpritAttack, enemyMaxAttack)) {
      dontCast = false;
      return new SkillTarget(hero, dontCast);
    }
...
class SEA_GOD_SKILL extends BaseSkill {
  ...
  getTarget(posibleGemSwaps, posibleSkillCasts, state){
    ...
    let enemyMaxHp = enemyPlayer.getCurrentMaxHp();
    var enemyHeros = enemyPlayer.getHerosAlive();
    // var enemyHero = enemyHeros.find(h => h.id == 'FIRE_SPIRIT');
    //
    let myHeros = state.getCurrentPlayer().getHerosAlive();
    ...
    // any enemy has skill
    let enemyHasSkill = enemyHeros.find(h => h.hasSkill());
    // neu k co ai co skill khong co ai giet dc minh turn tiep thi doi buff
    if (!enemyHasSkill && myHeros.length > 1 && this.hero.attack < enemyMaxHp) {// todo test
      // nen doi
      // truong hop co the kill tat ca thi dung luon

      return new SkillTarget(null, true);
      // truong hop xau bi bonus turn, nhung den thoi
    }
    // neu ben doi phuong khong co skill nao va
    // todo
    // if (this.hero.hp > Math.max(enemyHeroAttack, enemyMaxAttack) + 3) {// chua chet dc
    //   // Chua danh chet dc
...
class FIRE_SPIRIT_SKILL extends BaseSkill {
  getScore(state) {
    ...
    // neu con bo chet thi uu tien
    let enemyPlayer = state.getCurrentEnemyPlayer();
    let herosEnemy = enemyPlayer.getHerosAlive();
    let enemyMaxAttack = enemyPlayer.getCurrentMaxAttack();
    ...
    if (heroAllAttack && heroAllAttack.hasSkill() &&
        enemyMaxAttack >= heroAllAttack.hp) {// kha nang bi target cao
